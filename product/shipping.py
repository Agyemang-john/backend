...
def get_ip_address_from_request(request):
    ...
    Resolution order:
    1. X-Client-IP        — forwarded by our Next.js/Vercel SSR layer (real user IP from Vercel edge)
    ...
    5. REMOTE_ADDR         — last resort (will be Docker IP in containerised setups)
    """
    # 1. Our Next.js server reads the real user IP from Vercel's edge x-forwarded-for
    #    and passes it as X-Client-IP so Django sees the user's country, not Vercel's US IP.
    ssr_ip = request.META.get('HTTP_X_CLIENT_IP')
    if ssr_ip and is_valid_ip(ssr_ip):
        return ssr_ip
